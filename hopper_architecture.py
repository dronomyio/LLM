...
def draw_hopper_architecture():
    ...
    # Main GPU outline
    gpu_outline = patches.Rectangle((0, 0), 15, 11, linewidth=2, edgecolor=colors['border'], facecolor='white')
    ...
    for row in range(sm_rows):
        for col in range(sm_cols):
            ...
            sm = patches.Rectangle((x, y), sm_width, sm_height, linewidth=1, 
                                   edgecolor=colors['border'], facecolor=colors['sm'], alpha=0.8)
            ax.add_patch(sm)
            ...
            if row == 0 and col == 0:
                ...
                for tc_idx in range(4):
                    ...
                    tc = patches.Rectangle((tc_x, tc_y), 0.7, 0.3, linewidth=1, 
                                          edgecolor=colors['border'], facecolor=colors['tensor_core'])
                    ax.add_patch(tc)
    ...
    ax.text(14, 5, "...", fontsize=24, ha='center', va='center', color=colors['border'])
    
    # L2 Cache
    l2_height = 0.8
    l2_cache = patches.Rectangle((1, 2), 13, l2_height, linewidth=1,
                               edgecolor=colors['border'], facecolor=colors['l2'])
    ...
    hbm = patches.Rectangle((1, 1), 13, hbm_height, linewidth=1,
                           edgecolor=colors['border'], facecolor=colors['hbm'])
    ax.add_patch(hbm)
    ...
    expanded_sm = patches.Rectangle((expanded_sm_x, expanded_sm_y), expanded_sm_width, expanded_sm_height,
                                   linewidth=1.5, edgecolor=colors['border'], facecolor=colors['sm'], alpha=0.9)
    ...
    for i in range(4):
        ...
        tensor = patches.Rectangle((x_pos, y_pos), tensor_width, tensor_height,
                                  linewidth=1, edgecolor=colors['border'], facecolor=colors['tensor_core'])
        ax.add_patch(tensor)
    ...
    reg = patches.Rectangle((reg_x, reg_y), 2.4, 0.25, linewidth=1,
                           edgecolor=colors['border'], facecolor=colors['reg_file'])
    ax.add_patch(reg)
    ...
    highlight_components = [
        {"name": "TMA for Efficient\nMemory Transfer", "x": 12, "y": 9, "component": "TMA"},
        {"name": "Tensor Cores for\nFP8 Matrix Multiply", "x": 12, "y": 8, "component": "Tensor Core"},
        {"name": "Shared Memory for\nData Tiling", "x": 12, "y": 7, "component": "Shared Mem"},
        {"name": "Warp Specialization", "x": 12, "y": 6, "component": "Warp"}
    ]
    
    for i, comp in enumerate(highlight_components):
        # Create colored circle
        color = colors[comp["component"]]
        circle = plt.Circle((comp["x"] - 0.5, comp["y"]), 0.15, color=color, ec=colors['border'])
        ax.add_patch(circle)
    ...
    for comp in highlight_components:
        # Find target point in the SM
        if comp["component"] == "TMA":
            target_x, target_y = expanded_sm_x + 0.75, expanded_sm_y + 2.0
        elif comp["component"] == "Tensor Core":
            target_x, target_y = expanded_sm_x + 1.0, expanded_sm_y + 1.2
        elif comp["component"] == "Shared Mem":
            target_x, target_y = expanded_sm_x + 2.1, expanded_sm_y + 2.0
        elif comp["component"] == "Warp":
            target_x, target_y = expanded_sm_x + 1.5, expanded_sm_y + 0.8
    ...
    tc_outline = patches.Rectangle((1, 1), 8, 4, linewidth=2, 
                                  edgecolor=colors['border'], facecolor=colors['tensor_core'], alpha=0.9)
    ...
    for i in range(4):
        ...
        mma_unit = patches.Rectangle((x, y), 3, 1.5, linewidth=1, 
                                   edgecolor=colors['border'], facecolor='white', alpha=0.9)
    ...
    ax2.text(5, 0.5, info_text, ha='center', va='center', fontsize=10, 
             bbox=dict(boxstyle="round,pad=0.5", fc='white', ec=colors['border'], alpha=0.9))
    ...
    ax2.add_patch(patches.Rectangle((matrix_x, matrix_y), matrix_size, matrix_size, 
                                   facecolor=colors['sm'], alpha=0.8, ec=colors['border']))
    ...
    ax2.add_patch(patches.Rectangle((matrix_x + matrix_size*2.7, matrix_y), matrix_size, matrix_size, 
                                   facecolor=colors['tensor_core'], alpha=0.8, ec=colors['border']))
    ...
    ax2.add_patch(patches.Rectangle((matrix_x, matrix_y - 0.5), scale_width, scale_height, 
                                   facecolor=colors['tma'], alpha=0.9, ec=colors['border']))
